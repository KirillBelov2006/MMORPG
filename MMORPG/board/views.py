...
class PostItem(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        elif self.request.user == Announcement.objects.get(pk=self.kwargs.get('pk')).author:
...
class EditPost(PermissionRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        author = Announcement.objects.get(pk=self.kwargs.get('pk')).author.username
        if self.request.user.username == 'admin' or self.request.user.username == author:
            return super().dispatch(request, *args, **kwargs)
        else:
            return HttpResponse("Редактировать запись может только автор")
...
class DeletePost(PermissionRequiredMixin, DeleteView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.request.user.username == 'admin' or self.request.user.username == author:
            ...
            return HttpResponse("Удалить запись может только автор")
...
class Responses(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        global title
        if self.kwargs.get('pk') and Announcement.objects.filter(id=self.kwargs.get('pk')).exists():
            title = str(Announcement.objects.get(id=self.kwargs.get('pk')).title)
            print(title)
        context['form'] = ResponsesFilterForm(self.request.user, initial={'title': title})
        context['title'] = title
        if title:
            post_id = Announcement.objects.get(title=title)
...
class Respond(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        respond.post = Announcement.objects.get(id=self.kwargs.get('pk'))
        respond.save()
